class PoolsController < ApplicationController
  ...
  def show
    ...
    @service_plans = ServicePlan.where(company_id: current_user.try(:company_id)).order(id: :asc)
    @custom_field = CustomField.where(:user_id=>current_user.id ,:applies_to=>"pool")
  ...
  def create
    ...
    if @pool.save
      pool_address()
      begin
        current_user.custom_fields(params[:custom_field],  @pool.id)
      rescue
      end
    end
  ...
  def service_plan
    if @pool && @pool.update(:service_plan_id => params[:pool][:service_plan])
  ...
  def save_results
    ...
    if @pool && params[:chemical]
      params[:chemical].each do |val|
        if val && val[1]
          already = PoolChemicalItem.where(chamical_item_id: val[1][:id], pool_id: @pool.try(:id))
          if already.blank?
            PoolChemicalItem.create(chamical_item_id: val[1][:id], pool_id: @pool.try(:id),value: val[1][:value])
          else
            last = already.last
            if  last && last.update(value: val[1][:value])
